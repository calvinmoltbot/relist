...
// ---------------------------------------------------------------------------
// getDashboardData — the full payload the Dashboard page renders. Used by
// the Server Component (src/app/page.tsx) for first paint and by the API
// route (src/app/api/dashboard/route.ts) for client-side refreshes.
// ---------------------------------------------------------------------------
...
export interface DashboardData {
  ...
  dailyPlan: Awaited<ReturnType<typeof buildDailyPlan>>;
}
...
export async function getDashboardData(): Promise<DashboardData> {
  ...
  const [
    ...
    db
      ...
      .from(items)
      .where(and(eq(items.status, "listed"), lt(items.listedAt, fourteenDaysAgo))),

    db
      .select({ soldPrice: items.soldPrice, costPrice: items.costPrice })
      .from(items)
      .where(
        and(
    ...
    db
      .select({ soldPrice: items.soldPrice, soldAt: items.soldAt })
      .from(items)
      .where(
        and(
          or(eq(items.status, "sold"), eq(items.status, "shipped")),
  ...
  return {
    ...
    lastBackupAt: lastBackupRow[0]?.value ?? null,
    actions: {
